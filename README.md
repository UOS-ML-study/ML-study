...
- 팀원:
  - 강민석 (PM)
    - 역할: 모델링(기본 CNN 설계, 실험 플랜), 발표(전체 흐름 및 방법론 발표)
  - 김대원 
    - 역할: 데이터(데이터 정리, 라벨 분포 분석, Augmentation 설계), 발표 (EDA 및 데이터 파트)
  - 김미라 
    - 역할: 모델링(하미퍼파라미터 튜닝, 여러 CNN 구조 비교), 발표 (모델 성능 비교 파트)
  - 김수환 
    - 역할: 데이터(추가 데이터셋 조사/정리), LLM (LLM 활용 : 결과 설명문 자동 생성)
  - 이보람 
    - 역할: 발표 (데모, 데이터셋, 한계, 윤리 이슈 정리)
